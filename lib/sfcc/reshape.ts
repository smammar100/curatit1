...
export function reshapeCategory(
  ...
  return {
    ...
    parentCategoryTree: category.parentCategoryTree?.map((p) => ({
      id: p.id || "",
      name: p.name || "",
    })) || [],
    updatedAt: "",
...
export function reshapeImages(
  ...
  const imagesWithVariationAttributes = [...largeGroup].map((group) => {
    const variationAttributes = group.variationAttributes;
  ...
  return imagesWithVariationAttributes.map(({image, variationAttributes}) => {
    return {
      altText: image.alt!,
      url: image.disBaseLink || image.link,
      width: image.width || 1200,
      height: image.height || 1200,
      selectedOptions: variationAttributes?.reduce((acc, attr) => {
        const value = attr.values?.[0]?.value;
        if (!attr.id || !value) return acc
        acc.push({
          name: attr.id,
          value,
        });
  ...
  return {
    ...
    selectedOptions:
      Object.entries(variant.variationValues || {}).map(([key, value]) => ({
        name:
          product.variationAttributes?.find((attr) => attr.id === key)?.value ||
          key,
        value:
